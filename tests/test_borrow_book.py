...
def test_borrow_book_valid_input():
    ...
    with patch('library_service.get_book_by_id', return_value={"id": 1, "title": "Test Book", "available_copies": 2}), \
...
def test_borrow_book_no_available_copies():
    ...
    with patch('library_service.get_book_by_id', return_value={"id": 1, "title": "Test Book", "available_copies": 0}):
        success, message = borrow_book_by_patron("123456", 1)
...
def test_borrow_book_max_limit_reached():
    ...
    with patch('library_service.get_book_by_id', return_value={"id": 1, "title": "Test Book", "available_copies": 2}), \
         patch('library_service.get_patron_borrow_count', return_value=6):  # Over the limit (6 > 5)
...
def test_borrow_book_database_error():
    ...
    with patch('library_service.get_book_by_id', return_value={"id": 1, "title": "Test Book", "available_copies": 2}), \
         patch('library_service.get_patron_borrow_count', return_value=2), \
         patch('library_service.insert_borrow_record', return_value=False):  # Database error
